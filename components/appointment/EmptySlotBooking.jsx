...
const EmptySlotBooking = ({ checkError, chatHandler }) => {
  return (
    <>
      <div
        style={{
        ...
        <div className="flex flex-col items-center justify-center w-full p-4 md:mt-4 overflow-y-hidden">
          <div className="flex flex-col w-11/12 md:w-9/12 lg:w-9/12 px-2 md:px-4 xl:px-4 py-6 bg-custom-lightgrey shadow-lg shadow-gray-500/50 rounded-2xl">
            <p className="font-sans mx-2 text-[24px] md:text-4xl lg:text-4xl font-[900] text-brand-dark">
              {!checkError &&
                "Currently all slots for your coach are booked. Kindly come back here to book a call later."}
              {checkError &&
                "This is not a registered number. Book a call with a coach is only available if your are a Traya customer. "}
            </p>
            <p className="font-sans p-3 text-[16px] md:text-2xl lg:text-[22px] font-[400] text-[#434343]">
              {!checkError &&
                "Sorry for the inconvenience. \n You can choose to instead contact your coach via chat."}
              {checkError && "Or try again with a registered number"}
            </p>
            <button
...
export default EmptySlotBooking;
