...
import {
  APP_DEV_TRAYA_DASHBOARD,
  APP_TRAYA_DASHBOARD,
  COOKIES_EXPIRY,
} from "../../constants/constants";
import Router from "next/navigation";
import { handleFreshUserAttributes } from "../../api/handleFreshDesk";
  ...
  const _submitReply = async () => {
    ...
    try {
      ...
      if (_res.status === 200 && !_res.data.latest_order_id) {
        // moengageTrackEvent("website_hairtest_draft", {
        ...
        //   Email: _user.email,
        //   Mobile: _user.phone_number,
        //   "First name": _user.first_name.split(" ").shift(),
        //   "Last name": lastname ? lastname : _user.first_name,
        //   FORM_FILL_STARTED: "Yes",
        // });

        // handleFreshUserAttributes({
        //   Email: _user.email,
        //   "Form Status": "draft",
        //   "Synthetic Id": _res.data.syntheticId,
        //   "Case Id": _res.data.caseId,
        // });
        ...
        Router.push(
          `${APP_TRAYA_DASHBOARD}?${questions.phone_number.reply.replace(
            "+91",
            ""
          )}`
        );
        ...
        // Router.push(`http://localhost:3001/login?${questions.phone_number.reply.replace("+91",'')}`);
      }
      if (_res.status === 500) {
