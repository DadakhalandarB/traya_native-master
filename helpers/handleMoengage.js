...
import { MOENGAGE_APP_ID, MOENGAGE_AUTH_KEY } from "../constants/config";
import { fetchRequest } from "./fetchRequest";
import { getURL } from "next/dist/shared/lib/utils";
import { SEND_ACTIVITY_DATA_BACKEND } from "../constants/urls";
...
export const handleMoengage = (key, response, source) => {
...
export const addUserAndEventToMoengage = async (data) => {
	try {
	  const payload = {
		'type': 'transition',
		'elements': [
		  {
			type: 'customer',
			customer_id: data.mobile,
			attributes: {
			  name: data.name ? data.name : '',
			  email: data.name ? data.name : '',
			  form_status: data.form_status,
			  mobile: data.mobile,
			  latest_order_id: data.latest_order_id,
			  latest_order_count: data.latest_order_count,
	  ...
	  const headers = {
		...
		Authorization: MOENGAGE_AUTH_KEY,
	  };
  
	  const url = `https://api-03.moengage.com/v1/transition/${MOENGAGE_APP_ID}`;
	  ...
	  if (response.ok) {
		...
		return Promise.reject(await generateInternalServerErrorRepsonse(errorData, 'addUserAndEventToMoengage'));
	  }
	} catch (err) {
	  console.error('Error in addUserAndEventToMoengage', err.message);
	  let message = 'Something went wrong';
	  if (err.message) {
		message = err.message;
	  } else if (err.function || err.stack) {
	  ...
	  const _error = {
		...
		message: message,
		function: err.function || 'addUserAndEventToMoengage',
		stack: err.stack,
	  };
